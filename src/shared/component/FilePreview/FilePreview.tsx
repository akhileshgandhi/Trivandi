...
const getSiteUrlFromPath = (fileUrl: string): string => {
  if (!fileUrl) return '';
  ...
  if (decoded.startsWith('http')) {
    try {
      ...
      return getSiteUrlFromPath(urlObj.pathname);
    } catch {
      // Fallback
    }
  }
...
export class FilePreview extends React.Component<IFilePreviewProps, IFilePreviewState> {
  ...
  private _buildPreviewUrl = (
    ...
    if (['office'].includes(fileType)) {
      ...
      const dynamicSitePath = getSiteUrlFromPath(serverRelativeUrl);
      let baseSiteUrl = siteUrl || '';

      if (dynamicSitePath) {
        baseSiteUrl = `${window.location.protocol}//${window.location.host}${dynamicSitePath}`;
      }

      if (!baseSiteUrl) return fileUrl;

      const trimmedBase = baseSiteUrl.replace(/\/$/, '');
      const previewUrl = action === 'edit'
        ? `${trimmedBase}/_layouts/15/Doc.aspx?sourcedoc=${encodeURIComponent(serverRelativeUrl)}&action=edit`
        : `${trimmedBase}/_layouts/15/WopiFrame.aspx?sourcedoc=${encodeURIComponent(serverRelativeUrl)}&action=embedview`;
      ...
      case 'text':
        return (
          <div className={styles.textContainer}>
            ...
            />
              <div className={styles.hideBottomToolbar}></div>
          </div>
        );
