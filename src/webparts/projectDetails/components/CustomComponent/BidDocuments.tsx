...
const BidDocuments: React.FC<IBidDocumentsProps> = ({
  projectId,
  projectCode,
  projectTitle,
  businessProjectId,
  ...
  const getCurrentLibraryRootPath = async (): Promise<string> => {
    let baseLibUrl = bidDocumentsUrl;
  ...
  useEffect(() => {
    const hasUrls = bidDocumentsUrl;
    if (libraryName || hasUrls || businessProjectId) {
      void loadDocuments(1);
      void updateBreadcrumbs();
    }
  }, [projectId, currentFolderPath, libraryName, bidDocumentsUrl, businessProjectId]);
  ...
  const handleCreateFolder = async (folderName: string): Promise<void> => {
    try {
      let targetUrl: string = bidDocumentsUrl;
  ...
  const hasAnyUrl = bidDocumentsUrl || libraryName;
  ...
  return (
    <>
      <div className={styles.documentsWrapper}>
        {loading && <GlobalLoader variant="overlay" />}
